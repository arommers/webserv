...
// const fetchRandomPoke = function()
// {
...
//     })
//     .then(poke => console.log(poke.name))
//     .catch(error => console.error(error))
//     .finally(() => console.log('we made another funny!'));
// }

// fetchRandomPoke();

const fetchPokeEvo = function(name)
{
  return fetch(`https://pokeapi.co/api/v2/pokemon/${name}`)
  .then(function(response)
  {
    if (!response.ok)
      throw new Error(`could not retrieve info about ${name}`);
    return response.json();
  })
  .then(data => 
  {
    data = `https://pokeapi.co/api/v2/evolution-chain/${data.id}/`;
    return fetch(data);
  })
  .then(function(response)
  {
    if (!response.ok)
      throw new Error(`could not retrieve evo chain of ${name}`);
    return response.json();
  })
  .then(evoData => 
  {
    const evoChain = [];
    evoData.evolves_to.forEach(value => {evoChain.push(value)}
    )
